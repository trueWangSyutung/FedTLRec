...
- **联邦学习架构**: 实现了去中心化的模型训练，在保护用户隐私的同时利用多方数据提升模型效果
- **K-means聚类**: 使用K-means算法对用户进行聚类，使相似用户在同一组内共享信息
- **CoRA技术**: 采用低秩适应（LoRA）技术减少通信开销，提高训练效率
- **个性化推荐**: 每个用户拥有个性化的嵌入表示，同时从全局知识中受益
